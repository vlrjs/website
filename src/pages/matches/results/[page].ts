...
async function getMatchResults(page: string) {
    ...
    const results = games.map((_, idx) => {
        ...
        const data: any = {
            ...
            winner: score[0] === 2 ? teams[0] : teams[1],
            link: `https://vlr.gg${links[idx]}`,
            vods: {
                twitch: !!matches.twitchVod[idx],
                youtube: !!matches.youtubeVod[idx]
            }
        };
        if (status !== "LIVE") data.timeFromCompletion = eta, data.live = 0;
        if (tag) data.score = score
        return data
    })
    return results;
}
